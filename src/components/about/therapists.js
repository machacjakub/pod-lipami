export const therapists = [
    {
        name: 'Helena Mazancová, DiS.',
        bio: `<div>
                <p>Práce fyzioterapeuta mi dává smysl více jak 20 let. Klientům nabízím odborné znalosti, zkušenosti, ale
                také empatii. Pokud pomůžeme tělu správným způsobem na správném místě, výsledky jsou znát mnohdy překvapivě
                rychle.</p>
                <p>Při terapii se mi nejvíce osvědčila viscerální manipulace, neurální manipulace, osteopatie a kraniosakrální
                terapie. Těmito celostními přístupy lze jemně, avšak efektivně, ošetřit nejen svaly, klouby a fascie, ale i
                ovlivnit funkci vnitřních orgánů a komplexní fungování celého těla.</p>
                <p>V posledních letech se zabývám i senzorickou integrací u dětí se speciálními potřebami, baví mě pomáhat dětem,
                dospívajícím i dospělým při smyslovém přetížení.</p>
            </div>`,
        photo: 'helena.webp'
    },
    {
        name: 'Bc. Kateřina Štěpánková',
        bio: `<div>
                <p>Fyzioterapii jsem vystudovala na 2. lékařské fakultě v Praze pod vedením prof. Pavla Koláře a jeho týmu.
                    Studium jsem ukončila v roce 2005.</p>
                <p>Svou bezmála 20letou praxi v ambulantní rehabilitaci doplňuji absolvováním odborných kurzů.</p>
                <p>Při terapii vycházím zejména z mobilizačních technik páteře a žeber a ošetření svaloviny pánevního dna
                    metodou L. Mojžíšové. Ráda do své praxe zařazuji prvky z jógové terapie a poznatky o myofasciálních
                    řetězcích. V současné době se vzdělávám v metodě DNS dle prof. Pavla Koláře. Při terapii velmi ráda
                    využívám balanční pomůcky jako BOSU, míč a roller.</p>
                <p>Pracuji především s dospělými klienty.</p>
            </div>`,
        photo: 'katerina.webp'
    },
    {
        name: 'Bc. Eliška Lankašová Hancová',
        bio: `<div>
                <p>Jsem celoživotní sportovkyně a pohyb je nedílnou součástí mého života. Přes sportovní, trenérskou kariéru
                a vlastní zranění jsem začala prahnout po vzdělání v oblasti zdraví a prevence.</p>
            <p>Následoval roční rekvalifikační kurz se státní zkouškou, kde jsem získala titul pohybové specialistky a
                instruktorky Pilates. Fyzioterapii jsem vystudovala na Univerzitě Jana Evangelisty Purkyně v Ústí nad
                Labem.</p>
            <p>Nyní se věnuji metodám DNS, ACT, PNF, SMS, SPS, BPP a v rámci dalšího kurzu začínám s metodou
                Spiraldynamik. Nejvíce mě baví práce s dětmi a se sportovci.</p>
            </div>`,
        photo: 'eliska.webp'
    },
    {
        name: 'Mgr. Irena Horáčková',
        bio: `<div></div><p>Jsem fyzioterapeut s dlouholetou praxí, pracuji jak s dětskými tak s dospělými pacienty. Na fyzioterapii mě baví neustálé hledání odpovědí na otázky, které nám klade naše tělo. Těší mě, když se podaří vtáhnout do tohoto procesu i pacienta, protože jedině tak muže dojít ke zlepšení.</p>
                <p>Obor jsem vystudovala na FTVS UK. Mým prvním pracovištěm byla dětská léčebna Vesna v Janských Lázních. V Mladé Boleslavi jsem pracovala v několika ambulantních zařízeních a před 2 lety si založila i svou soukromou praxi.</p>
                <p>Mojí oblíbenou metodou je dynamická neuromuskulární stabilizace, kterou ráda kombinuji s prvky jógy. Zajímají mě i vztahy pohybového aparátu a vnitřních orgánů. Věnuji se podologii, spolupracuji se Studiem zdravého obouvání, které vyrábí individuální vložky do bot.</p>
                <p>V práci s dětmi využívám Vojtovu metodu i DNS. V posledních letech jsem se začala vzdělávat v terapii senzorické integrace, která nabízí širší pohled na pohybový vývoj dítěte. Snažím se pracovat vždy s myšlenkou, že nejlepší motivace pro dítě je hra.</p>
           </div>`
    },
    {
        name: 'Hanka Vytlačilová, DiS.',
        bio: `<p>Dětský fyzioterapeut.</p>`
    },
    {
        name: 'Bc. Kateřina Maťáková',
        bio: `<div>
                <p>Jsem absolventkou oboru fyzioterapie na Zdravotně sociální fakultě Jihočeské univerzity. Po ukončení studia jsem nastoupila do FN v Motole, konkrétně na oddělení neurologie. Dále moje kroky vedly na Kliniku Dr. Pírka, kde jsem sbírala zkušenosti na ortopedickém oddělení a na ambulanci. V ambulantní praxi pracuji jak s dospělou klientelou, tak s dětmi. Ke každému se snažím přistupovat individuálně s ohledem na jeho aktuální zdravotní stav. Zároveň chci klienta motivovat k péči o fyzické zdraví tím, že se naučí lépe vnímat své tělo a pohyb. Při práci nejčastěji využívám prvky z metody DNS, ACT, spirální stabilizaci páteře, nespecifické mobilizace a kinesiotaping.</p>
                <p>V ambulantní praxi pracuji jak s dospělou klientelou, tak s dětmi. Ke každému se snažím přistupovat individuálně s ohledem na jeho aktuální zdravotní stav. Zároveň chci klienta motivovat k péči o fyzické zdraví tím, že se naučí lépe vnímat své tělo a pohyb. Při práci nejčastěji využívám prvky z metody DNS, ACT, spirální stabilizaci páteře, nespecifické mobilizace a kinesiotaping.</p>
              </div>`
    }
]